      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
            {formData.values.map((val, index) => (
                <div key={val.id} className="p-4 border rounded-lg space-y-2">
                    <Label>Value {index + 1}</Label>
                    <Input value={val.title} onChange={(e) => handleValueChange(index, 'title', e.target.value)} placeholder="Title (e.g. Precision)" />
                    <Textarea value={val.description} onChange={(e) => handleValueChange(index, 'description', e.target.value)} placeholder="Description" />
                </div>
            ))}
        </div>
